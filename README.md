...
| **Hybrid-rewrite** (one Codex call per module) | **75.9%** (924/1217) — **2.11x** | 9.6% | 39.4% | 60.8% | **97.6%** (160/164) | 0.708 |
...
**Headline:** hybrid-rewrite recovery on **1217 modules / 492,209 LoC**:
...
- **Declaration match: 1208/1217 (99.3%)** — signature match plus every module/class-level variable and annotated attribute by name.
- **Strict match: 924/1217 (75.9%)** — full stripped-AST equality (cosmetic regression telltale; bounded by CPython compiler normalisations).
- **Behavioral smoke: 740/1217 (60.8%)** — recovered module imports under the producing interpreter and exposes the same public name + signature surface as the original. The semantic axis that tolerates the most compiler normalisations; see [Why not naïve pyc → py → pyc?](#why-not-naïve-pyc--py--pyc) for what `BX`/`BN`/`BS` measure and what each one catches.
...
- **Edit similarity (mean): 0.708** — Decompile-Bench-style character-level Ratcliff-Obershelp ratio averaged over the corpus. 1.0 means byte-identical, 0.0 means entirely dissimilar. A continuous metric that surfaces incremental rule-pass improvements which haven't yet flipped any boolean axis.
...
| **stdlib-full**<br/>_Full Python 3.14 stdlib (single-file modules)_ | 153 | 130,182 | 153/153 (100.0%) | 151/153 (98.7%) | 150/153 (98.0%) | 132/153 (86.3%) | 48/153 (31.4%) | 73/153 (47.7%) | 126/153 (82.4%) | n/a | 0.785 |
...
| **pypi-top20**<br/>_PyPI top-20 pure-Python packages_ | 682 | 260,908 | 682/682 (100.0%) | 682/682 (100.0%) | 680/682 (99.7%) | 429/682 (62.9%) | 19/682 (2.8%) | 130/682 (19.1%) | 381/682 (55.9%) | n/a | 0.627 |
...
| **aggregate** | **1217** | **492,209** | **1213/1217 (99.7%)** | **1211/1217 (99.5%)** | **1208/1217 (99.3%)** | **924/1217 (75.9%)** | **117/1217 (9.6%)** | **479/1217 (39.4%)** | **740/1217 (60.8%)** | **160/164 (97.6%)** | **0.708** |
...
@software{pychd,
  ...
  note   = {Two-tier evaluation on 1{,}217 real-world modules
            / 492,209 LoC spanning the Python 3.14 stdlib, 26
            ...
            strict\_match metric used by prior research): 75.9\%
            strict-AST match (2.11$\times$ improvement over the
